...
async def test_index_redirects_to_login(client):
...
async def test_register_page_has_form(client):
    resp = await client.get("/register")
...
async def test_dashboard_has_category_sidebar(client):
    resp = await client.get("/dashboard")
    assert 'id="category-list"' in resp.text
    assert 'id="add-category-form"' in resp.text


async def test_dashboard_has_task_section(client):
    resp = await client.get("/dashboard")
    assert 'id="task-list"' in resp.text
    assert 'id="task-form"' in resp.text


async def test_dashboard_has_filters(client):
    resp = await client.get("/dashboard")
...
async def test_js_served(client):
...
async def test_login_includes_navbar(client):
    resp = await client.get("/login")
    assert 'class="navbar' in resp.text
    assert 'Task Manager' in resp.text


async def test_login_includes_css(client):
    resp = await client.get("/login")
    assert "/static/css/style.css" in resp.text


async def test_login_includes_js(client):
    resp = await client.get("/login")
    assert "/static/js/app.js" in resp.text


async def test_pages_link_to_each_other(client):
    login_resp = await client.get("/login")
    assert '/register' in login_resp.text

    register_resp = await client.get("/register")
